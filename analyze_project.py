...
def analyze_project(root_dir):
    ...
    # ─── Промт для ИИ ───
    report.append("\n\n" + "=" * 50)
    report.append("ПРОМТ ДЛЯ ИИ:")
    report.append("=" * 50)
    report.append("Проанализируй проект, ознакомься с задачей, которую тебе написал пользователь. "
                  "Далее запроси файлы необходимые для более детального анализа и решения проблемы. "
                  "Если не хватает каких-то файлов, всегда сначала запрашивай их. "
                  "Код пиши только после одобрения пользователя. "
                  "После проделанной работы всегда описывай кратко ожидаемый результат, "
                  "чтобы пользователь всегда мог проверить и сравнить ожидание и реальный результат. "
                  "Если пользователь говорит что что-то работает, а что-то нет — "
                  "сначала сравни реализацию работающего и неработающего элемента, найди разницу и сделай так же. "
                  "Не спорь с наблюдениями пользователя. "
                  "Не пытайся подтянуть наблюдения пользователя под свою теорию — "
                  "строй теории на основе фактов, а не факты на основе теорий.")
    report.append("=" * 50)
